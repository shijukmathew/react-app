pipeline {
    ...
    stages {
        ...
        stage('Build Docker Image') {
            ...
            steps {
                script {
                    app = docker.build("skm125/react-app")
                    app.inside {
        ...
        stage('Push Docker Image') {
            ...
            steps {
                script {
                    docker.withRegistry('https://registry.hub.docker.com', 'skm125') {
         ...
         stage('DeployToStaging') {
            ...
            steps {
                    script {
                        sh "docker pull skm125/react-app:${env.BUILD_NUMBER}"
                        try {
                        ...
                        sh "docker run --restart always --name react-app -p 1233:80 -d skm125/react-app:${env.BUILD_NUMBER}"
                    }
            }
        }
        ...
        stage('DeployToProduction') {
            ...
            steps {
                input 'Does the staging environment look OK? Did You get 200 response?'
                 milestone(1)
                    script {
                        sh "docker pull shijukmathewb/react-app:${env.BUILD_NUMBER}"
                        try {
                            sh "docker stop react-app"
                            sh "docker rm react-app"
                        } catch (err) {
